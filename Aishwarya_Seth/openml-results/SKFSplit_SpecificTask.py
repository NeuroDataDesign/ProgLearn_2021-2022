...
for row in tasks.iterrows():
    ...
    task = openml.tasks.get_task(row[1]['tid'])
    ...
    try: 
        ...
        mean_rf_accuracy.append(np.mean(accuracy_rf))
        std_rf_accuracy.append(np.std(accuracy_rf))
        mean_kdf_accuracy.append(np.mean(accuracy_kdf))
        std_kdf_accuracy.append(np.std(accuracy_kdf))
        name_of_dataset.append(row[1]['name'])
        task_id_of_dataset.append(row[1]['tid'])

        print("Completed ", row[1]['name'])

        df = pd.DataFrame()
        df['task_id'] = task_id_of_dataset
        df['dataset name'] = name_of_dataset
        df['mean accuracy rf'] = mean_rf_accuracy
        df['std accuracy rf'] = std_rf_accuracy
        df['mean accuracy kdf'] = mean_kdf_accuracy
        df['std accuracy kdf'] = std_kdf_accuracy
        
        df.to_csv('OpenML_CC18_UpdatedSKFSplit.csv')
    except(ValueError):
        print(ValueError)
        mean_rf_accuracy.append(None)
        std_rf_accuracy.append(None)
        mean_kdf_accuracy.append(None)
        std_kdf_accuracy.append(None)
        name_of_dataset.append(row[1]['name'])
        task_id_of_dataset.append(row[1]['tid'])
...
# df['task_id'] = task_id_of_dataset
# df['dataset name'] = name_of_dataset
# df['accuracy kdf'] = mean_kdf_accuracy
# df['accuracy rf'] = mean_rf_accuracy
# df.to_csv('OpenML_CC18_UpdatedSKFSplit.csv')
