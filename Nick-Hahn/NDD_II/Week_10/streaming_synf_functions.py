...
def stream_ble_v_angle(angle_sweep, task1_sample, task2_sample, mc_rep):
    mean_ble = np.zeros(45, dtype=float)
    for i, angle in enumerate(angle_sweep):
        print("angle {}".format(angle))
        ble=0
        for j in range(mc_rep):
            ble += stream_ble_v_angle_exp(angle * np.pi / 180, task1_sample, task2_sample)
        mean_ble[i] = ble / mc_rep
    write_result("ble_v_angle/stream_100rep.txt", mean_ble)
    return mean_ble


def stream_ble_v_angle_exp(angle, task1_sample, task2_sample):
    synf_single_task = LifelongClassificationForest(default_n_estimators=20, default_max_depth=ceil(log2(task1_sample)))
    synf_two_task = LifelongClassificationForest(default_n_estimators=10,default_max_depth=ceil(log2(task1_sample)))
    x, y = generate_gaussian_parity(50)
    x2, y2 = generate_gaussian_parity(50,angle_params=angle)
    x_test, y_test = generate_gaussian_parity(1000)
    synf_single_task.add_task(x, y, n_estimators=20, task_id=0)
    synf_two_task.add_task(x, y, n_estimators=10, task_id=0)
    for i in range(int(task1_sample / 50) - 1):
        x, y = generate_gaussian_parity(50)
        synf_single_task.update_task(x, y, task_id=0, inputclasses=[0,1], n_estimators=20)
        synf_two_task.update_task(x, y, task_id=0, inputclasses=[0,1], n_estimators=10)
    synf_two_task.add_task(x2, y2, n_estimators=10, task_id=1)
    for i in range(int(task1_sample / 50) - 1):
        x2, y2 = generate_gaussian_parity(50, angle_params=angle)
        synf_two_task.update_task(x2, y2, task_id=1, inputclasses=[0,1], n_estimators=10)
...
def stream_ble_v_nsamples(task1_samples, task2_samples, task2_angle, mc_rep):
    mean_ble = np.zeros((len(task1_samples), int(task2_samples/50)), dtype=float)
    for i, task1_sample in enumerate(task1_samples):
        ble = np.zeros((1,int(task2_samples/50)),dtype=float)
        for j in range(mc_rep):
            ble += stream_ble_v_nsamples_exp(task1_sample, task2_samples, task2_angle)
        mean_ble[i,:] = ble/mc_rep
    return mean_ble


def stream_ble_v_nsamples_exp(task1_sample, task2_samples, task2_angle):
    ble = np.zeros((1,int(task2_samples/50)),dtype=float)
    synf_single_task = LifelongClassificationForest(default_n_estimators=20, default_max_depth=ceil(log2(task1_sample)))
    synf_two_task = LifelongClassificationForest(default_n_estimators=10,default_max_depth=ceil(log2(task1_sample)))
    x, y = generate_gaussian_parity(50)
    x2, y2 = generate_gaussian_parity(50,angle_params=task2_angle)
    x_test, y_test = generate_gaussian_parity(1000)
    ...
    for i in range(int(task1_sample / 50) - 1):
        ...
        synf_two_task.update_task(x, y, task_id=0, inputclasses=[0,1], n_estimators=10)
    synf_two_task.add_task(x2, y2, n_estimators=10, task_id=1)
    for i in range(int(task2_samples / 50) - 1):
        x2, y2 = generate_gaussian_parity(50, angle_params=task2_angle)
        synf_two_task.update_task(x2, y2, task_id=1, inputclasses=[0,1], n_estimators=10)
        y1_hat = synf_single_task.predict(x_test, task_id=0)
        y2_hat = synf_two_task.predict(x_test, task_id=0)
        single_task_error = 1 - np.mean(y1_hat == y_test)
        two_task_error = 1 - np.mean(y2_hat == y_test)
        ble[0,i+1] = single_task_error/two_task_error
    return ble
